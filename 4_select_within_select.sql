...
WHERE area > ALL(SELECT area FROM world WHERE continent = 'Europe' )
...
WHERE population >= ALL(SELECT population*3 FROM world y WHERE x.continent = y.continent > 0 AND x.name <> y.name)
